...
function next_turn() {
	...
	setTimeout(() => {
		const condition = m.querySelector('.blackjack') || (m.querySelector('.busted') && m.querySelectorAll('.busted, .done').length + 1 === players.length) || m.querySelectorAll('.done').length === players.length;
